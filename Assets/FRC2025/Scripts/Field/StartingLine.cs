...
namespace FRC2025
{
    ...
    public class StartingLine : ZoneBase
    {
        ...
        /// <summary>
        /// Handles the logic when a collider enters the trigger area.
        /// </summary>
        /// <remarks>This method updates the internal state for the entering object based on its instance
        /// ID. If the object has not been awarded points, it initializes its state. If the object has already been
        /// awarded the maximum points, the points are deducted, and its state is reset.</remarks>
        /// <param name="other">The <see cref="Collider"/> representing the object that entered the trigger area.</param>
        ...
        /// </summary>
        /// <remarks>This method checks the score status of the object associated with the collider and,
        /// if applicable, awards points to the appropriate alliance. Points are only awarded if the object has not
        /// already been scored or has a score of zero.</remarks>
        ...
        protected override void OnLeave(Collider other)
        {
            ...
            if (awardedPoints == -1 || awardedPoints == 0)
            {
                ScoreManager.AddScore(_points, _allianceColor);
        ...
        /// application.</remarks>
        protected override void OnAutoEnd()
        {
            Destroy(this);
        }
